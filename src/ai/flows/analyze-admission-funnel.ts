...
const analyzeAdmissionFunnelPrompt = ai.definePrompt({
  ...
  **Analysis Task:**
  1.  **Calculate Conversion Rates:** For each period (This Month, Last Month), calculate the conversion rate between each stage (e.g., Enquiries to Forms Filled, Forms Filled to Joined) and the overall conversion rate (Enquiries to Joined).
  ...
  **Suggestions Task:**
  ...
  For example, if the bottleneck is between "Enquiries" and "Forms Filled", suggest:
  - "Simplify the enquiry form on the website."
  - "Implement an automated SMS/WhatsApp follow-up for new enquiries with a direct link to the application form."

  If the bottleneck is between "Forms Filled" and "Joined", suggest:
